...
(define-inline (%mask size) (- (arithmetic-shift 1 size) 1))
(define-inline (extract-bit-field size pos n)
  (arithmetic-shift (bitwise-and n (arithmetic-shift (%mask size) pos))
                    (- pos)))
...
(define-constant *outside-char* 99) ; luft-balloons
(define-constant *pad-char* 101)    ; dalmations

(define-constant *base64-decode-table*
...
(define-inline (base64-decode-char c)
  (vector-ref *base64-decode-table* (char->integer c)))

(define-constant *base64-encode-table*
...
(define-constant decode-src-length
  (lcm 76 78))

(define-constant decode-dst-length
  (* 3 (arithmetic-shift (+ 3 decode-src-length) -2)))

(define-constant encode-src-length
  (* 3 1024))
...
;;   we get to a valid character.  However, most Scheme compilers are
      ...
      (if (<= len first-max-col)
          ...
          (string-append
           (if (positive? first-max-col)
               ...
               "")
           (string-intersperse (string-chop (substring str first-max-col)
                                            effective-max-col)
