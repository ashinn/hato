...
(define (number->u32-string n)
  (string-append
   (hex (bitwise-and n #xFF))
   (hex (bitwise-and (arithmetic-shift n -8) #xFF))
   (hex (bitwise-and (arithmetic-shift n -16) #xFF))
   (hex (bitwise-and (arithmetic-shift n -24) #xFF))))
...
(define (md5-digest M)
  (let* ((len (string-length M))
         (N (quotient (+ len 3) 4))
         (lim (quotient N 16))
         (pad (- 64 (modulo (- len 55) 64)))
         (M (string-append
             M
             (string (integer->char #x80))
             (make-string pad (integer->char 0))
             (string (integer->char (bitwise-and (* len 8) #xFF))
                     (integer->char (arithmetic-shift (* len 8) -8))
                     (integer->char 0) (integer->char 0)
                     (integer->char 0) (integer->char 0)
                     (integer->char 0) (integer->char 0))))
         (X (make-vector 16 0))
         (A A)
    ...
    (do ((i 0 (+ i 1)))
        ((> i lim)
         (string-append (number->u32-string A) (number->u32-string B)
                        (number->u32-string C) (number->u32-string D)))
      ;; Copy block i into X.
      (do ((j 0 (+ j 1)))
          ((= j 16))
        (vector-set! X j (string-u32-ref M (* (+ (* i 16) j) 4))))
